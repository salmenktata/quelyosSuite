...
export function validateThemeConfig(config: unknown): ValidationResult {
  ...
  const errors = (validateTheme.errors || []).map((error: ErrorObject) => {
    ...
    switch (error.keyword) {
      case 'required':
        return `${path} : Propriété manquante "${((error as any).params).missingProperty}"`;
      case 'pattern':
        return `${path} : Format invalide (attendu: ${((error as any).params).pattern})`;
      case 'enum':
        return `${path} : Valeur invalide (attendu: ${((error as any).params).allowedValues?.join(', ')})`;
      case 'type':
        return `${path} : Type invalide (attendu: ${((error as any).params).type})`;
      case 'minLength':
        return `${path} : Trop court (min: ${((error as any).params).limit} caractères)`;
      case 'additionalProperties':
        return `${path} : Propriété non autorisée "${((error as any).params).additionalProperty}"`;
      default:
        return `${path} : ${message}`;
    }
  });
