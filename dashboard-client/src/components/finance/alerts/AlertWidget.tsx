...
export default function AlertWidget() {
  ...
  async function loadRecentTriggers() {
    try {
      ...
      // Extraire tous les triggers récents (dernières 48h)
      ...
      const last48h = allTriggers.filter((t) => {
        ...
        return diff < 48 * 60 * 60 * 1000; // 48 heures
      });
    ...
    <GlassCard className="p-4 sm:p-6">
      ...
      <div className="space-y-3">
        {recentTriggers.map((trigger) => (
          <div
            key={trigger.id}
            className="p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg hover:bg-amber-500/15 transition-colors"
          >
