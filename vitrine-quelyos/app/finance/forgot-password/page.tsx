...
export default function ForgotPasswordPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${config.api.finance}/auth/password/forgot`, {
        method: 'POST',
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      <div className="pointer-events-none fixed inset-0">
        <div className="absolute left-1/4 top-0 h-[500px] w-[500px] rounded-full bg-indigo-500/10 blur-[150px]" />
        ...
        {!sent ? (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="h-12 w-full rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 font-semibold text-white shadow-lg shadow-indigo-500/25 transition-all hover:shadow-indigo-500/40 disabled:cursor-not-allowed disabled:opacity-50"
              >
          ...
          <div className="text-center">
            ...
            <p className="mb-8 text-slate-400">
              Si un compte existe avec l&apos;adresse <span className="font-medium text-white">{email}</span>, vous recevrez un email avec les instructions pour réinitialiser votre mot de passe.
            </p>
            <Link
              href="/finance/login"
