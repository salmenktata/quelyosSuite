...
import {
  ArrowRight, Sparkles, Briefcase, Users, Clock, Brain,
  TrendingUp, Heart, Wallet, Megaphone, FolderKanban, Check
} from 'lucide-react';
...
const modules = [
  ...
  { icon: FolderKanban, label: 'Projets', color: 'text-indigo-400' },
  { icon: Wallet, label: 'Facturation', color: 'text-emerald-400' },
  { icon: TrendingUp, label: 'Finance', color: 'text-amber-400' },
  { icon: Megaphone, label: 'Marketing', color: 'text-violet-400' },
];

const processSteps = [
  { title: 'Prospection & CRM', description: 'Pipeline commercial visuel, scoring leads, relances automatiques. Ne laissez plus aucune opportunité filer.', icon: '🎯' },
  { title: 'Gestion de projet', description: 'Kanban, timetracking, jalons. Suivez chaque mission du brief à la livraison avec une vue claire.', icon: '📋' },
  { title: 'Facturation automatique', description: 'Devis transformé en facture en un clic, relances automatiques, suivi des paiements en temps réel.', icon: '💰' },
  { title: 'Fidélisation & croissance', description: 'Satisfaction client, upsell automatique, rapports de rentabilité par client et par projet.', icon: '📈' },
];

export default async function ServicesSolutionPage() {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-indigo-950/20 to-slate-950">
      ...
      <section className="relative overflow-hidden py-24 sm:py-36">
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute left-1/3 bottom-10 h-[350px] w-[350px] animate-pulse rounded-full bg-pink-500/6 blur-3xl [animation-delay:2s]" />
        </div>
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-2 backdrop-blur-sm">
              ...
              <span className="text-sm font-medium text-indigo-300">Solution Services B2B</span>
            </div>

            <h1 className="text-4xl font-bold tracking-tight sm:text-6xl lg:text-7xl">
              <span className="text-white">Développez votre </span>
              <span className="bg-gradient-to-r from-indigo-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">activité</span>
              <span className="text-white"> sereinement</span>
            </h1>

            <p className="mt-6 text-lg leading-8 text-slate-300 sm:text-xl">{solution.valueProp}</p>

            <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-sm text-slate-400">
              <span className="flex items-center gap-2"><Users className="h-4 w-4 text-indigo-400" />200+ agences</span>
      ...
      <PainPointsGrid painPoints={solution.painPoints} title="Les défis des agences et services B2B" subtitle="Des problématiques que nous comprenons et auxquelles nous apportons des réponses concrètes" className="bg-slate-950/50" />
      <FeaturesGrid features={solution.features} title={`Tout ce qu'il faut pour ${solution.verb}`} subtitle="Les outils essentiels des professionnels du service" />
      <ProcessMetier steps={processSteps} title="Comment ça fonctionne au quotidien" subtitle="De la prospection à la fidélisation, un flux B2B automatisé" className="bg-slate-950/50" />
      <TestimonialsGrid testimonials={solution.testimonials} title="Ce que disent les professionnels" subtitle="Découvrez comment ils ont transformé leur activité" className="bg-slate-950/30" />
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl rounded-3xl border border-indigo-500/20 bg-gradient-to-br from-indigo-950/50 to-violet-950/50 p-12 text-center backdrop-blur-sm sm:p-16">
            ...
            <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">Prêt à développer votre activité ?</h2>
