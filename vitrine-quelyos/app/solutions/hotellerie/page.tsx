...
import {
  ArrowRight, Sparkles, Hotel, Users, Clock, Brain,
  TrendingUp, CalendarDays, Wallet, Heart, Megaphone, Check
} from 'lucide-react';

const SECTOR_ID = 'hotellerie';

export const metadata: Metadata = {
  title: 'Quelyos Hotel - Solution pour hôtellerie',
  description: 'Remplissez vos chambres, simplifiez votre gestion. Réservations, planning, check-in : solution hôtelière.',
  keywords: ['solution hôtellerie', 'gestion hôtel', 'logiciel hôtel', 'réservations hôtel', 'CRM hôtellerie', 'facturation hôtel'],
  openGraph: {
    title: 'Quelyos Hotel - Solution pour hôtellerie',
    description: 'Remplissez vos chambres, simplifiez votre gestion. Réservations, planning, check-in : solution hôtelière.',
    url: 'https://quelyos.com/solutions/hotellerie',
    type: 'website',
    images: [{ url: '/og-image.png', width: 1200, height: 630, alt: 'Quelyos Hotel - Solution hôtellerie complète' }],
  },
  alternates: { canonical: 'https://quelyos.com/solutions/hotellerie' },
};

const modules = [
  { icon: CalendarDays, label: 'Réservations', color: 'text-indigo-400' },
  { icon: Hotel, label: 'Planning', color: 'text-amber-400' },
  { icon: Wallet, label: 'Finance', color: 'text-emerald-400' },
  { icon: Heart, label: 'CRM', color: 'text-pink-400' },
  { icon: Megaphone, label: 'Marketing', color: 'text-violet-400' },
];

const processSteps = [
  { title: 'Réservation en ligne', description: 'Moteur de réservation intégré, synchronisation OTAs, tarification dynamique automatique.', icon: '📅' },
  { title: 'Check-in fluide', description: 'Pré-enregistrement en ligne, attribution chambres automatique, accueil personnalisé avec historique client.', icon: '🔑' },
  { title: 'Service & facturation', description: 'Extras, room service, minibar ajoutés au séjour. Facture consolidée générée automatiquement au départ.', icon: '🧾' },
  { title: 'Fidélisation & avis', description: 'Email post-séjour, programme fidélité, collecte avis. Transformez chaque client en ambassadeur.', icon: '⭐' },
];

export default async function HotellerieSolutionPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-indigo-950/20 to-slate-950">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-2 backdrop-blur-sm">
              ...
              <span className="text-sm font-medium text-amber-300">Solution Hôtelière Complète</span>
            </div>

            <h1 className="text-4xl font-bold tracking-tight sm:text-6xl lg:text-7xl">
              <span className="text-white">Transformez votre </span>
              <span className="bg-gradient-to-r from-amber-400 via-yellow-400 to-orange-400 bg-clip-text text-transparent">établissement</span>
              <span className="text-white"> hôtelier</span>
            </h1>

            <p className="mt-6 text-lg leading-8 text-slate-300 sm:text-xl">{solution.valueProp}</p>

            <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-sm text-slate-400">
              <span className="flex items-center gap-2"><Users className="h-4 w-4 text-amber-400" />120+ hôteliers</span>
      ...
      <PainPointsGrid painPoints={solution.painPoints} title={`Les défis de l'hôtellerie`} subtitle="Des problématiques que nous comprenons et auxquelles nous apportons des réponses concrètes" className="bg-slate-950/50" />
      <FeaturesGrid features={solution.features} title={`Tout ce qu'il faut pour ${solution.verb}`} subtitle={`Les outils essentiels des professionnels de l'hôtellerie`} />
      <ProcessMetier steps={processSteps} title="Comment ça fonctionne au quotidien" subtitle="De la réservation au check-out, une expérience fluide et automatisée" className="bg-slate-950/50" />
      <TestimonialsGrid testimonials={solution.testimonials} title={`Ce que disent les ${solution.sectorName.toLowerCase()}`} subtitle="Découvrez comment ils ont transformé leur établissement" className="bg-slate-950/30" />
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl rounded-3xl border border-amber-500/20 bg-gradient-to-br from-amber-950/50 to-orange-950/50 p-12 text-center backdrop-blur-sm sm:p-16">
            ...
            <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">Prêt à transformer votre établissement ?</h2>
            <p className="mt-6 text-lg leading-8 text-slate-300">Rejoignez {solution.stats.clients} hôteliers qui nous font confiance.</p>
