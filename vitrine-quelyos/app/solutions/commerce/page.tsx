...
export default async function CommerceSolutionPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      ...
      <section className="relative overflow-hidden py-20 sm:py-32">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h1 className="text-4xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-6xl">{solution.headline}</h1>
            <p className="mt-6 text-xl leading-8 text-slate-600 dark:text-slate-400">{solution.valueProp}</p>
      ...
      <PainPointsGrid painPoints={solution.painPoints} title="Les défis du commerce" className="bg-slate-50 dark:bg-slate-900/50" />
      <FeaturesGrid features={solution.features} title={`Tout ce qu'il faut pour ${solution.verb}`} />
      <TestimonialsGrid testimonials={solution.testimonials} title={`Ce que disent les ${solution.sectorName.toLowerCase()}`} className="bg-slate-50 dark:bg-slate-900/50" />
      ...
      <section className="py-20 bg-gradient-to-br from-indigo-50 to-violet-50 dark:from-indigo-950/30 dark:to-violet-950/30">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-4xl">Prêt à transformer votre commerce ?</h2>
            <div className="mt-10">
              <Link href="/contact?utm_source=solution-cta&utm_package=commerce" className="inline-block rounded-lg bg-indigo-600 px-8 py-4 text-base font-semibold text-white shadow-sm hover:bg-indigo-500">Demander une démonstration</Link>
            </div>
          </div>
        </div>
      </section>
