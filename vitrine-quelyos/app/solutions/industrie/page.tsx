...
import {
  ArrowRight, Sparkles, Factory, Users, Clock, Brain,
  TrendingUp, Wrench, Package, Wallet, Shield, Check
} from 'lucide-react';
...
const modules = [
  { icon: Wrench, label: 'GMAO', color: 'text-amber-400' },
  { icon: Package, label: 'Stock', color: 'text-emerald-400' },
  { icon: Wallet, label: 'Finance', color: 'text-indigo-400' },
  { icon: Users, label: 'RH', color: 'text-pink-400' },
  { icon: Shield, label: 'Qualité', color: 'text-violet-400' },
];

const processSteps = [
  { title: 'Maintenance préventive', description: 'Planification automatique basée sur les heures machine, alertes avant panne, historique complet des interventions.', icon: '🔧' },
  { title: 'Suivi production', description: 'Ordres de fabrication, suivi des cadences, taux de rendement. Pilotez votre production en temps réel.', icon: '🏭' },
  { title: 'Stock pièces détachées', description: 'Seuils de réappro automatiques, traçabilité par lot, coûts de possession optimisés par IA.', icon: '📦' },
  { title: 'Pilotage financier', description: 'Coût par unité produite, marges par ligne, budget maintenance. Toute la data pour décider vite.', icon: '📊' },
];
...
export default async function IndustrieSolutionPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-indigo-950/20 to-slate-950">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-2 backdrop-blur-sm">
              ...
              <span className="text-sm font-medium text-amber-300">Solution Industrielle Complète</span>
            </div>

            <h1 className="text-4xl font-bold tracking-tight sm:text-6xl lg:text-7xl">
              <span className="text-white">Optimisez votre </span>
              <span className="bg-gradient-to-r from-amber-400 via-orange-400 to-red-400 bg-clip-text text-transparent">production</span>
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-sm text-slate-400">
              <span className="flex items-center gap-2"><Users className="h-4 w-4 text-amber-400" />80+ industriels</span>
      ...
      <PainPointsGrid painPoints={solution.painPoints} title={`Les défis de l'industrie`} subtitle="Des problématiques que nous comprenons et auxquelles nous apportons des réponses concrètes" className="bg-slate-950/50" />
      <FeaturesGrid features={solution.features} title={`Tout ce qu'il faut pour ${solution.verb}`} subtitle="Les outils essentiels des industriels qui réussissent" />
      <ProcessMetier steps={processSteps} title="Comment ça fonctionne au quotidien" subtitle="De la maintenance à la production, un flux industriel automatisé" className="bg-slate-950/50" />
      <TestimonialsGrid testimonials={solution.testimonials} title="Ce que disent les industriels" subtitle="Découvrez comment ils ont transformé leur production" className="bg-slate-950/30" />
      ...
      <section className="py-20">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-3xl rounded-3xl border border-amber-500/20 bg-gradient-to-br from-amber-950/50 to-orange-950/50 p-12 text-center backdrop-blur-sm sm:p-16">
            ...
            <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">Prêt à transformer votre production ?</h2>
            <p className="mt-6 text-lg leading-8 text-slate-300">Rejoignez {solution.stats.clients} industriels qui nous font confiance.</p>
