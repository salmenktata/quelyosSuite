...
/**
 * CountdownTimer - Timer urgence pour offres limitées
 *
 * Fonctionnalités :
 * - Countdown temps réel (mise à jour 1s)
 * - 3 variants: badge (compact card), compact (inline), full (prominent)
 * - Auto-hide quand expiré
 * - Callback onExpire
 * - Animation pulse pour urgence
 *
 * @example
 * ```tsx
 * <CountdownTimer
...
export function CountdownTimer({
  ...
  if (variant === 'badge') {
    return (
      <div className={clsx(
        ...
        'bg-red-600 text-white px-2.5 py-1 rounded-md',
        'text-xs font-bold',
        className
      )}>
  ...
  if (variant === 'compact') {
    return (
      <div className={clsx('inline-flex items-center gap-2 text-sm', className)}>
        ...
        <div className="flex items-center gap-1 font-mono font-bold text-gray-900 dark:text-white">
          ...
          <span className="text-gray-400 dark:text-gray-600">:</span>
          <span className="text-red-600 dark:text-red-400">{formatNumber(timeRemaining.seconds)}s</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={clsx(
      ...
      'border-2 border-red-200 dark:border-red-800 rounded-xl p-4',
      className
    )}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900 dark:text-white">⚡ Offre à durée limitée</p>
            <p className="text-xs text-gray-600 dark:text-gray-400">Profitez-en avant qu&apos;il ne soit trop tard !</p>
          </div>
        </div>
      </div>
...
function TimeBox({ value, label, highlight = false }: { value: number; label: string; highlight?: boolean }) {
  return (
    <div className="flex flex-col items-center">
      <div className={clsx(
        'rounded-lg shadow-md px-3 py-2 min-w-[60px] text-center transition-all duration-300',
        highlight ? 'bg-red-600 text-white animate-pulse' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white'
      )}>
        <span className="text-2xl font-bold font-mono">
          {value.toString().padStart(2, '0')}
        </span>
      </div>
      <span className="text-xs text-gray-600 dark:text-gray-400 font-medium mt-1">{label}</span>
    </div>
  )
}
...
function Separator() {
  return (
    <div className="text-2xl font-bold text-gray-400 dark:text-gray-600 pb-6">:</div>
  )
}
